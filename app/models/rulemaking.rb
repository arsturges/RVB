  ...
  def find_milestone(phase, activity, date_category, revision_number )
      ...
      @lookup_table = {}
        milestones.by_revision(revision_number).each do |m|
          @lookup_table["#{m.phase_id}|#{m.activity_id}|#{m.date_category_id}"] = m
        end
      end
      @lookup_table["#{phase}|#{activity}|#{date_category}"]
   end


  def selectmilestone(phase, activity, date_category, revision_number)
    milestone = find_milestone(phase, activity, date_category, revision_number)
